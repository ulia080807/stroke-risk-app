...
const MyRiskApp = {
    ...
    calculateBMI: function() {
        ...
        if (height && weight && height > 0 && weight > 0) {
            ...
            let bmiCategory = '', bmiColor = '#2A5C8A';
            if (bmi < 18.5) { bmiCategory = ' (Недостаточный вес)'; bmiColor = '#ff9800'; }
            else if (bmi < 25) { bmiCategory = ' (Нормальный вес)'; bmiColor = '#4CAF50'; }
            else if (bmi < 30) { bmiCategory = ' (Избыточный вес)'; bmiColor = '#FFC107'; }
            else { bmiCategory = ' (Ожирение)'; bmiColor = '#F44336'; }
            bmiResult.textContent = bmi.toFixed(1) + bmiCategory;
    ...
    updateProgress: function() {
        ...
        requiredFields.forEach(field => {
            if (field.type === 'number' && field.value !== '' && parseFloat(field.value) > 0) filledCount++;
            else if (field.type === 'select-one' && field.value !== '') filledCount++;
        });
    ...
    validateFormData: function(data) {
        if (!data.age || data.age < this.minAge || data.age > 120) {
            return { valid: false, message: `Пожалуйста, введите корректный возраст (от ${this.minAge} до 120 лет)` };
        }
        ...
        for (let field of requiredFields) {
            if (!field.field || field.field === '') {
                return { valid: false, message: `Пожалуйста, заполните поле: "${field.name}"` };
            }
            if (field.min && field.max && (field.field < field.min || field.field > field.max)) {
                return { valid: false, message: `Пожалуйста, введите корректное значение для "${field.name}" (от ${field.min} до ${field.max})` };
            }
        }
    ...
    calculateComprehensiveRisk: function(data) {
        ...
        const hasEstablishedCVD = (data.strokeHistory === 'yes' || data.cadHistory === 'yes' || data.atherosclerosis === 'yes');
        let primaryRisk = 0;
        let score2Risk = null, framinghamRisk = null, abcd2Risk = null, chads2vascRisk = null;
        
        if (hasEstablishedCVD) {
            abcd2Risk = this.calculateABCD2Risk(data);
            chads2vascRisk = this.calculateCHADS2VAScRisk(data);
            primaryRisk = 3.0;
            if (data.strokeHistory === 'yes') primaryRisk += abcd2Risk.sixMonthRisk * 0.5;
            if (data.afibHistory === 'yes') primaryRisk += chads2vascRisk.sixMonthRisk * 0.5;
        ...
        if (primaryRisk < 1) { riskLevel = 'low'; probability = 'низкий'; }
        else if (primaryRisk < 5) { riskLevel = 'moderate'; probability = 'умеренный'; }
        else if (primaryRisk < 10) { riskLevel = 'high'; probability = 'высокий'; }
        else { riskLevel = 'very-high'; probability = 'очень высокий'; }
        
        const riskFactors = this.identifyRiskFactors(data, bmi);
        return {
            sixMonthRisk: primaryRisk.toFixed(2), riskLevel, probability, factors: riskFactors,
            bmi: bmi.toFixed(1), hasEstablishedCVD, adjustments, score2Risk, framinghamRisk, abcd2Risk, chads2vascRisk
        };
    },
    
    calculateSCORE2Risk: function(data, bmi) {
        let ageGroup = '40-49';
        if (data.age < 40) ageGroup = '40-49';
        ...
        let baseRisk = this.SCORE2_COEFFICIENTS.baseRisk[ageGroup][sexKey][smokingIndex];
        if (data.age < 40) baseRisk *= 0.3;
        ...
        return {
            ageGroup, baseRisk, tenYearRisk: adjustedRisk,
            riskCategory: adjustedRisk < 2.5 ? 'низкий/умеренный' : adjustedRisk < 7.5 ? 'высокий' : 'очень высокий'
        };
    },
    ...
    calculateFraminghamRisk: function(data) {
        ...
        if (data.diabetes === 'yes') {
            points += 3;
            if (data.diabetesDuration === 'long') points += 1;
        ...
        return { points, tenYearRisk, riskCategory: points <= 10 ? 'низкий' : points <= 20 ? 'умеренный' : 'высокий' };
    },
    ...
    calculateABCD2Risk: function(data) {
        ...
        let sixMonthRisk = 3.0;
        if (score <= 3) sixMonthRisk = 3.0;
        else if (score <= 5) sixMonthRisk = 9.0;
        else sixMonthRisk = 15.0;
        return { score, sixMonthRisk, riskCategory: score <= 3 ? 'низкий' : score <= 5 ? 'умеренный' : 'высокий' };
    },
    ...
    calculateCHADS2VAScRisk: function(data) {
        ...
        const riskMap = {0:0.0,1:1.3,2:2.2,3:3.2,4:4.0,5:6.7,6:9.8,7:9.6,8:12.5,9:15.2};
        ...
        return { score, strokeRisk, sixMonthRisk, anticoagulation };
    },
    
    calculateAdjustments: function(data, bmi) {
        let total = 0;
        const details = [];
        if (data.activity === 'sedentary') { total += 0.5; details.push({ factor: 'Малоподвижный образ жизни', adjustment: '+0.5%' }); }
        if (bmi >= 35) { total += 0.8; details.push({ factor: 'Ожирение III степени (ИМТ ≥ 35)', adjustment: '+0.8%' }); }
        else if (bmi >= 30) { total += 0.4; details.push({ factor: 'Ожирение (ИМТ 30-34.9)', adjustment: '+0.4%' }); }
        if (data.alcohol === 'heavy') { total += 0.6; details.push({ factor: 'Злоупотребление алкоголем', adjustment: '+0.6%' }); }
        if (data.ldlCholesterol && data.ldlCholesterol >= 4.9) { total += 0.5; details.push({ factor: 'ЛПНП ≥ 4.9 ммоль/л', adjustment: '+0.5%' }); }
        if (data.glucose && data.glucose >= 7.0) { total += 0.8; details.push({ factor: 'Глюкоза натощак ≥ 7.0 ммоль/л', adjustment: '+0.8%' }); }
        else if (data.glucose && data.glucose >= 6.1) { total += 0.4; details.push({ factor: 'Глюкоза натощак 6.1-6.9 ммоль/л', adjustment: '+0.4%' }); }
        if (data.hdlCholesterol && data.hdlCholesterol < 1.0 && data.gender === 'male') { total += 0.3; details.push({ factor: 'Низкий ЛПВП (< 1.0 ммоль/л)', adjustment: '+0.3%' }); }
        else if (data.hdlCholesterol && data.hdlCholesterol < 1.2 && data.gender === 'female') { total += 0.3; details.push({ factor: 'Низкий ЛПВП (< 1.2 ммоль/л)', adjustment: '+0.3%' }); }
        if (data.kidneyDisease === 'yes') { total += 1.5; details.push({ factor: 'Хроническая болезнь почек', adjustment: '+1.5%' }); }
        if (data.statins === 'yes') { total -= 0.3; details.push({ factor: 'Приём статинов', adjustment: '-0.3%' }); }
        if (data.anticoagulants === 'yes' && data.afibHistory === 'yes') { total -= 1.0; details.push({ factor: 'Приём антикоагулянтов при ФП', adjustment: '-1.0%' }); }
        if (data.activity === 'active') { total -= 0.2; details.push({ factor: 'Активный образ жизни', adjustment: '-0.2%' }); }
    ...
    displayRiskResult: function(result) {
        ...
        if (result.score2Risk) {
            scalesHTML += `<div class="score-card"><h4><i class="fas fa-globe-europe"></i> SCORE2 (Россия)</h4><div class="score-value">${result.score2Risk.tenYearRisk.toFixed(1)}%</div><p>10-летний риск ССЗ</p><p class="score-category">${result.score2Risk.riskCategory} риск</p></div>`;
        }
        if (result.framinghamRisk) {
            scalesHTML += `<div class="score-card"><h4><i class="fas fa-balance-scale"></i> Шкала Фрамингема</h4><div class="score-value">${result.framinghamRisk.points} баллов</div><p>10-летний риск: ${result.framinghamRisk.tenYearRisk}%</p><p class="score-category">${result.framinghamRisk.riskCategory} риск</p></div>`;
        }
        if (result.abcd2Risk) {
            scalesHTML += `<div class="score-card"><h4><i class="fas fa-heartbeat"></i> ABCD²</h4><div class="score-value">${result.abcd2Risk.score} баллов</div><p>Риск за 90 дней: ${result.abcd2Risk.sixMonthRisk}%</p><p class="score-category">${result.abcd2Risk.riskCategory} риск</p></div>`;
        }
        if (result.chads2vascRisk) {
            scalesHTML += `<div class="score-card"><h4><i class="fas fa-stethoscope"></i> CHA₂DS₂-VASc</h4><div class="score-value">${result.chads2vascRisk.score} баллов</div><p>Годовой риск: ${result.chads2vascRisk.strokeRisk}%</p><p><strong>Антикоагулянты:</strong> ${result.chads2vascRisk.anticoagulation}</p></div>`;
        }
        
        let adjustmentsHTML = '';
        if (result.adjustments && result.adjustments.details.length > 0) {
            adjustmentsHTML = `<div class="adjustments-list"><h4><i class="fas fa-sliders-h"></i> Дополнительные корректировки:</h4><ul>${result.adjustments.details.map(adj => `<li>${adj.factor}: <strong style="color: ${adj.adjustment.startsWith('+') ? '#F44336' : '#4CAF50'}">${adj.adjustment}</strong></li>`).join('')}</ul></div>`;
        }
        ...
        let resultHTML = `
            <div class="risk-result">
                <div class="risk-header">
                    <div>
                        ...
                        ${result.hasEstablishedCVD ? '<p class="badge badge-secondary">Вторичная профилактика (есть ССЗ)</p>' : '<p class="badge badge-primary">Первичная профилактика</p>'}
                    </div>
                ...
                <div class="risk-description">
                    ...
                    ${result.factors.length > 0 ? `<p><strong>Основные факторы риска:</strong> ${result.factors.join(', ')}</p>` : '<p>У вас мало факторов риска, что является хорошим знаком.</p>'}
                ...
                <div class="detailed-scores">
                    ...
                    <div class="scores-grid">${scalesHTML}</div>
                </div>
                ...
                <div class="recommendations">
                    ...
                    <div class="recommendation-category">
                        ...
                        <ul class="recommendation-list">${recommendations.medical.map(rec => `<li>${rec}</li>`).join('')}</ul>
                    </div>
                    <div class="recommendation-category">
                        <h4><i class="fas fa-walking"></i> Изменение образа жизни</h4>
                        <ul class="recommendation-list">${recommendations.lifestyle.map(rec => `<li>${rec}</li>`).join('')}</ul>
                    </div>
                    <div class="recommendation-category">
                        <h4><i class="fas fa-calendar-check"></i> Мониторинг и профилактика</h4>
                        <ul class="recommendation-list">${recommendations.monitoring.map(rec => `<li>${rec}</li>`).join('')}</ul>
                    </div>
                </div>
            ...
            <div class="form-actions" style="margin-top: 30px;">
                <button class="btn btn-secondary" id="recalculate-btn"><i class="fas fa-redo"></i> Пересчитать с новыми данными</button>
                <button class="btn btn-primary" id="education-btn"><i class="fas fa-graduation-cap"></i> Изучить признаки инсульта</button>
            </div>
        `;
        resultContainer.innerHTML = resultHTML;
        document.getElementById('recalculate-btn')?.addEventListener('click', () => { this.switchTab('questionnaire'); });
        document.getElementById('education-btn')?.addEventListener('click', () => { this.switchTab('education'); });
    },
    
    generateRecommendations: function(result) {
        const recommendations = { medical: [], lifestyle: [], monitoring: [] };
        if (result.hasEstablishedCVD) {
            recommendations.medical.push('Вы находитесь в группе вторичной профилактики — необходим регулярный приём назначенных препаратов', 'Консультация невролога/кардиолога не реже 2 раз в год');
        }
        if (result.riskLevel === 'low') {
            recommendations.medical.push('Продолжайте регулярные профилактические осмотры у терапевта 1 раз в год', 'Контролируйте артериальное давление не реже 1 раза в месяц');
            recommendations.lifestyle.push('Физическая активность ≥ 150 минут умеренной нагрузки в неделю', 'Сбалансированное питание: соль < 5 г/день, овощи и фрукты ≥ 400 г/день', 'Поддерживайте нормальный вес: ИМТ 18.5-24.9');
            recommendations.monitoring.push('Диспансеризация по ОМС ежегодно', 'Измерение АД 1 раз в месяц', 'Контроль веса и окружности талии');
        } else if (result.riskLevel === 'moderate') {
            recommendations.medical.push('Запишитесь на приём к терапевту в ближайшие 2-4 недели', 'Если АД ≥ 140/90 более 3 дней подряд — срочно к врачу', 'Рассмотрите консультацию кардиолога');
            recommendations.lifestyle.push('Приоритет: ежедневный контроль АД утром и вечером', 'Если курите — начните отказ: риск снижается через 24 часа', 'При избыточном весе: снижение на 5-10% снижает риск инсульта на 25%', 'Ограничьте алкоголь до 1 дозы/день');
            recommendations.monitoring.push('Дневник АД: измерения ежедневно', 'Контроль холестерина и глюкозы каждые 6 месяцев', 'Обращайте внимание на симптомы: головокружение, онемение, нарушение речи');
        } else {
            recommendations.medical.push('Немедленно запишитесь к терапевту или неврологу', 'При симптомах (онемение, нарушение речи, асимметрия лица) — звоните 103/112', 'Требуется: ЭКГ, УЗИ брахиоцефальных артерий, липидограмма, коагулограмма');
            recommendations.lifestyle.push('Срочный контроль АД: 2-3 раза в день', 'Немедленный отказ от курения', 'Снижение веса под контролем врача', 'Полное исключение алкоголя, соль ≤ 3 г/день');
            recommendations.monitoring.push('Ежедневный мониторинг АД с записью', 'Регулярный контроль липидного профиля и глюкозы', 'Изучите признаки инсульта и ТИА', 'Сообщите близким о вашем риске');
        }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    emergencyButtons.forEach(button => {
        ...
        button.addEventListener('mouseenter', function() {
            if (window.innerWidth > 768) this.style.transform = 'translateY(-5px)';
        });
